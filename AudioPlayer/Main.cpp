...
bool startMicChat()
{
	...
	while(TRUE) {
		...
		if (bytesReceived = recvfrom(micSocket, buffer, DATABUFSIZE, 0, (sockaddr*)&micServer, &size) == SOCKET_ERROR)
        {
...
DWORD WINAPI mcThread(LPVOID args)
{
	...
	char buff[STREAMBUFSIZE];
	int addr_size = sizeof(struct sockaddr_in);

    // open the stream
	if (!streamPlayer->OpenStream(1, 1, buff, STREAMBUFSIZE, sfPCM))
	{
	...
	while (TRUE)
	{
		...
		if ((nRet % 2) == 0) 
			streamPlayer->PushDataToStream(buff, nRet);
...
{
	char* tmp = new char[STREAMBUFSIZE];
